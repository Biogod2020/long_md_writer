...
## 🧠 项目专属规则 (Project Memory)

### 1. 强制性真实 API 测试 (MANDATORY Real API Testing)
*   **原则**：所有集成测试与关键功能测试**严禁使用 Mock**。必须通过真实的推理和多模态感知来验证逻辑。
*   **工具**：统一使用本地代理 `geminicli2api-async` (运行于 `http://localhost:8888/v1`)。
*   **目标**：确保 AI 的复杂推理能力、多模态指令遵循度和 SOTA 协议的闭环执行是真实可靠的。

### 2. 视觉终审机制 (Visual Quality Assurance)
*   **准则**：在任何任务或 Track 标记为“完成”前，必须对最终产物（Markdown 文稿或 HTML 渲染图）执行**视觉校验**。
*   **手段**：利用 Chrome DevTools (Playwright) 截取快照，并结合多模态 VLM 进行语义与视觉的一致性对比。

### 3. Context7 外部库协议
*   **流程**：涉及任何第三方库（如 LangGraph, Playwright）的 API 调用或环境配置时，**必须先调用 `resolve-library-id`** 获取最新 SOTA 文档，杜绝凭旧经验猜测。
