"""
Final Verification E2E - v16 (Comprehensive Stress Test)
Using v12 inputs (Deep technical writing + Mixed assets) 
to verify the integrated robustness of Validator + MarkdownQA.
"""
...
async def run_verification_v16():
    # SOTA: New version ID to avoid overwriting v12
    job_id = "v16_comprehensive_run"
    workspace_base = "workspace"
    workspace_path = Path(workspace_base) / job_id

    # SOTA: RESUME MODE ENABLED (Removed rmtree)
    if workspace_path.exists():
        print(f"🔄 Workspace found. Resuming E2E Run: {job_id}")
    else:
        print(f"🚀 [VERIFICATION v16] Starting FRESH E2E Run: {job_id}")
    ...
    print("⚙️  Initiating SOTA 2.0 Pipeline (High-Density Mode)...")
    ...
    print("\n" + "="*50)
    print("📊 VERIFICATION v16 FINAL AUDIT REPORT")
    print("="*50)
    ...
    # Check for Markdown Path Consistency (SOTA v15 requirement)
    merged_path = workspace_path / "final_full.md"
    if merged_path.exists():
        content = merged_path.read_text(encoding="utf-8")
        resource_refs = content.count('src="resource/')
        print(f"Global Resource Redirection: {resource_refs} images mapped.")

    if not state.errors and (svg_count > 0 or web_count > 0):
        print("\n🏆 [VERIFICATION v16] COMPREHENSIVE RUN PASSED.")
    else:
        print("\n💀 [VERIFICATION v16] FAILED. Check logs for details.")
        exit(1)

if __name__ == "__main__":
    asyncio.run(run_verification_v16())
