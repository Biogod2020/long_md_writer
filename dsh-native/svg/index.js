 ...
 *
 * The DSH plugin entrypoint supplies the harness-specific register function
 * and workspace resolver. This module neither imports DSH packages nor calls
 * a model, so the gate and submission contract remain portable.
 *
 * @module longwriter/svg-dsh-adapter
 */
...
function assertDependencies(registerTool, dependencies) {
  ...
  if (typeof dependencies?.appendVisualReview !== 'function') throw new TypeError('svg adapter requires appendVisualReview')
}
...
export function applySvg(registerTool, dependencies, config = {}) {
  ...
  registerTool({
    name: 'svg_preflight',
    description: 'Run deterministic geometry preflight on a registered planned SVG, render a retained assets/reviews/*.png preview, and append a hash-bound receipt. Inspect the returned preview_asset_path with a read_image-capable tool before recording a visual review. This tool never calls a model.',
    parameters: {
  ...
  registerTool({
    ...
    description: 'Append an explicit pass or fail review of a retained PNG preview after svg_preflight. The store binds the record to the exact SVG, preflight receipt, preview, and hashes; a passing review must confirm every required label. This tool records evidence and never calls a model.',
    parameters: {
      asset_id: { type: 'string', required: true, description: 'Registered planned SVG asset id.' },
      preflight_id: { type: 'string', required: true, description: 'Passing preflight id returned by svg_preflight.' },
      reviewer: { type: 'string', required: true, description: 'Person or independent reviewer identity that inspected the preview.' },
      verdict: { type: 'string', required: true, description: 'pass only after visual inspection; otherwise fail.' },
      summary: { type: 'string', required: true, description: 'Concrete review conclusion tied to the retained preview.' },
      findings: { type: 'json', description: 'Optional array of concise visual findings.' },
      checked_labels: { type: 'json', description: 'Array of every required plan label visibly confirmed in the preview.' },
    },
    isConcurrencySafe: () => false,
    execute(args, exec) {
      return recordAssetReview(dependencies.workspace(exec), args, { appendVisualReview: dependencies.appendVisualReview })
    },
  })
}
