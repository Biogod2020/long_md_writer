/**
 * Controlled visual-evidence workflow shared by the App Server tools and portable CLI.
 *
...
export function compactPreflightMessages(messages, label, maximum = 100) {
  if (!Array.isArray(messages)) throw new TypeError(`${label} must be an array`)
  if (messages.length <= maximum) return [...messages]
  if (!Number.isSafeInteger(maximum) || maximum < 2) throw new TypeError('preflight message maximum must be at least 2')
  const entries = messages.map((message, index) => ({
    index,
    message,
    category: String(message).split(':', 1)[0] || 'other',
  }))
  const selected = []
  ...
  selected.push(`${label}_truncated:${messages.length}>${maximum};omitted=${messages.length - selected.length};categories=${seenCategories.size}`)
  return selected
}
...
export async function preflightAsset(workspace, input, dependencies = {}) {
  try {
    ...
    const runPreflight = dependencies.preflightSvg ?? preflightSvg
    const rawReport = await runPreflight(svg, {
    ...
    const report = {
      ...rawReport,
      issues: compactPreflightMessages(rawReport.issues ?? [], 'preflight_issues'),
      warnings: compactPreflightMessages(rawReport.warnings ?? [], 'preflight_warnings'),
    }
    ...
    const receipt = await appendVisualPreflight(workspace, {
      ...
      design_metrics: report.metrics?.design,
    })
    return {
