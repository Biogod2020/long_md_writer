...
function validateMeasurements(value, expectedLength) {
  ...
  return value.map((metric, index) => {
    ...
    const missingCharacters = metric.missing_characters === undefined
      ? []
      : metric.missing_characters
    if (!Array.isArray(missingCharacters) || missingCharacters.some(item => typeof item !== 'string')) {
      throw new Error(`CoreText returned invalid missing glyph evidence at index ${index}`)
    }
    const resolvedFontName = typeof metric.resolved_font_name === 'string' && metric.resolved_font_name.trim()
      ? metric.resolved_font_name.trim()
      : null
    return {
      width,
      ascent,
      descent,
      resolved_font_name: resolvedFontName,
      missing_characters: [...new Set(missingCharacters)],
    }
  })
}
